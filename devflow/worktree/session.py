...
def _session_path(devflow_dir: Path) -> Path:
    return devflow_dir / SESSION_FILENAME
...
def save_worktree_session(
    ...
    path = _session_path(devflow_dir)
    if session is None:
...
def load_worktree_session(devflow_dir: Path) -> WorktreeSession | None:
    path = _session_path(devflow_dir)
    if not path.exists():
        return None
    try:

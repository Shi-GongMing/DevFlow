...
from devflow.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from devflow.commands.completion import CompletionPopup
from devflow.commands.handlers import register_all_commands
from devflow.config import MCPServerConfig, ProviderConfig
from devflow.hooks import HookContext, HookEngine, load_hooks
from devflow.conversation import ConversationManager, Message
from devflow.mcp import ConnectResult, MCPManager
from devflow.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from devflow.permissions import (
...
from devflow.agents.task_manager import TaskManager
from devflow.agents.trace import TraceManager
from devflow.agents.notification import inject_task_notifications
from devflow.commands.handlers.tasks import create_tasks_command
from devflow.skills.executor import SkillExecutor
from devflow.skills.loader import SkillLoader
from devflow.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from devflow.cache import FileCache
from devflow.tools import ToolRegistry, create_default_registry
from devflow.tools.agent_tool import AgentTool
from devflow.tools.ask_user import AskUserEvent, AskUserTool
from devflow.tools.impl.tool_search import ToolSearchTool
from devflow.tools.load_skill import LoadSkill
from devflow.worktree.cleanup import start_stale_cleanup_task
from devflow.worktree.manager import WorktreeManager
from devflow.commands.handlers.worktree import create_worktree_command
from devflow.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".devflow", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".devflow" / "history"
...
class DevFlowApp(App):
    ...
    def on_mount(self) -> None:
        self.register_theme(_DEVFLOW_THEME)
        self.theme = "devflow"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".devflow" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".devflow" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".devflow" / "permissions.local.yaml",
            ),
        ...
        from devflow.filehistory import FileHistory
        ...
        from devflow.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from devflow.config import WorktreeConfig
        ...
        from devflow.tools.enter_worktree import EnterWorktreeTool
        from devflow.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from devflow.teams.manager import TeamManager
        from devflow.tools.team_create import TeamCreateTool
        from devflow.tools.team_delete import TeamDeleteTool
        ...
        from devflow.commands.handlers.trace import create_trace_command
        ...
        from devflow.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from devflow.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from devflow.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from devflow.plan_dialog import InlinePlanWidget, PlanChoice
        from devflow.prompts import build_plan_mode_exit_reminder
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from devflow.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from devflow.permission_dialog import InlinePermissionWidget

...
from devflow.hooks import HookConfigError, HookEngine, load_hooks
from devflow.permissions import PermissionMode


def main() -> None:
    # 先确保 .devflow/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".devflow").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".devflow/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="devflow", description="DevFlow AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    if args.remote:
        from devflow.remote import RemoteServer
    ...
    from devflow.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    ...
    from devflow.client import create_client, resolve_context_window
    from devflow.conversation import ConversationManager
    from devflow.memory.instructions import load_instructions
    ...
    from devflow.tools import create_default_registry

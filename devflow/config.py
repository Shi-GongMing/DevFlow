...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".devflow" / "config.yaml",
        cwd / ".devflow" / "config.yaml",
        cwd / ".devflow" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .devflow/config.yaml "
            "in project or ~/.devflow/config.yaml"
        )
    return merged

...
from devflow.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from devflow.conversation import ThinkingBlock as ConvThinkingBlock
from devflow.memory.auto_memory import MemoryManager
from devflow.permissions import (
    Decision,
    PermissionChecker,
    PermissionMode,
)
from devflow.hooks import HookContext, HookEngine, ToolRejectedError
from devflow.hooks.engine import HookNotification
from devflow.prompts import build_environment_context, build_plan_mode_reminder, build_system_prompt
from devflow.tools import ToolRegistry
from devflow.tools.base import (
    MAX_OUTPUT_CHARS,
    StreamEnd,
    StreamEvent,
    TextDelta,
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".devflow" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".devflow" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from devflow.permissions.rules import Rule, extract_content
    ...
    def _maybe_persist_or_truncate(self, tool_use_id: str, text: str) -> str:
        from devflow.context.manager import (

...
class TestPathSandbox:
    ...
    def test_temp_dir_allowed(self) -> None:
        tmp = Path(tempfile.gettempdir()) / "devflow_test.txt"
...
class TestRuleEngine:
    ...
    def test_append_local_rule(self) -> None:
        tmpdir = Path(tempfile.mkdtemp())
        local_path = tmpdir / ".devflow" / "permissions.local.yaml"
...
class TestPermissionChecker:
    ...
    def test_dangerous_command_denied(self) -> None:
        from devflow.tools.bash import Bash
        tool = Bash()
    ...
    def test_write_path_outside_sandbox_asks(self) -> None:
        from devflow.tools.write_file import WriteFile
        tool = WriteFile()
    ...
    def test_read_path_outside_sandbox_asks(self) -> None:
        from devflow.tools.read_file import ReadFile
        tool = ReadFile()
...
async def test_e2e_rule_allows_git():
    ...
    tmpdir = Path(tempfile.mkdtemp())
    rules_file = tmpdir / ".devflow" / "permissions.yaml"
